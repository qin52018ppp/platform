 ...
 * @Auth:秦盼（Q）
 * @Description:秦盼（Q）
 * @Date:Created in 17:46 2018/3/13
 * @Modified By:
 */
public class BigFileReader {
    ...
    private IFinishHandle finishHandle;
    ...
    private BigFileReader(File file, IHandle handle, String charset, int bufferSize, int threadSize,IFinishHandle finishHandle) {
    ...
    public void start() {
        ...
        cyclicBarrier = new CyclicBarrier(startEndPairs.size(), new Runnable() {
            public void run() {
                System.out.println("use all time: " + (System.currentTimeMillis() - startTime));
        ...
        for (StartEndPair pair : startEndPairs) {
            System.out.println("slice nums：" + pair);
    ...
    private class SliceReaderTask implements Runnable {
        ...
        public void run() {
            try {
                ...
                cyclicBarrier.await();//使线程置入休眠直到最后一个线程的到来之后唤醒所有休眠的线程
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    public static class Builder {
        ...
        public Builder(String file, IHandle handle, IFinishHandle finishHandle) {
        ...
        public BigFileReader build() {
            return new BigFileReader(this.file, this.handle, this.charset, this.bufferSize, this.threadSize,finishHandle);
        }
    }
}
